...
function ExpensesTable({ expenses, onEdit, onDelete }: { expenses: Expense[]; onEdit: (e: Expense) => void; onDelete: (e: Expense) => void }) {
  return (
            ...
            <tr key={e.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
              ...
              <td className="px-4 py-3">
                <div className="flex items-center justify-center gap-1">
                  <button onClick={() => onEdit(e)} className="p-1.5 rounded-lg text-[#C8FF00] hover:bg-[#C8FF00]/10 transition-colors" title="Editar"><Edit2 className="w-4 h-4" /></button>
                  <button onClick={() => onDelete(e)} className="p-1.5 rounded-lg text-red-400 hover:bg-red-400/10 transition-colors" title="Eliminar"><Trash2 className="w-4 h-4" /></button>
                </div>
              </td>
            </tr>
          ))}
...
function ExpenseFormModal({ isOpen, editItem, register, errors, saving, onClose, onSubmit }: {
  isOpen: boolean; editItem: Expense | null; register: any; errors: any; saving: boolean; onClose: () => void; onSubmit: (e?: React.BaseSyntheticEvent) => void;
}) {
  if (!isOpen) return null;
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={editItem ? 'Editar gasto' : 'Nuevo gasto'} size="sm">
  ...
  return (
    <div className="space-y-6">
      ...
      {expenses.length === 0 ? (
        ...
        <ExpensesTable expenses={páginated} onEdit={openEdit} onDelete={setDeleteTarget} />
      )}
      ...
      <ExpenseFormModal isOpen={modalOpen} editItem={editItem} register={register} errors={errors} saving={saving} onClose={() => setModalOpen(false)} onSubmit={handleSubmit(onSubmit)} />
