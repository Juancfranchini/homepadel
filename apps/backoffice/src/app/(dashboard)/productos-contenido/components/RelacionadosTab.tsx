...
interface Props {
  ...
  control: any;
  showRelated: boolean;
  onToggleShowRelated: () => void;
}

export default function RelacionadosTab({ products, selectedId, setValue, control, showRelated, onToggleShowRelated }: Props) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <p className="text-sm text-gray-500">Selecciona productos para mostrar como relacionados.</p>
        <div className="flex items-center gap-2"><Toggle checked={showRelated} onChange={onToggleShowRelated} /><span className="text-xs text-gray-400">{showRelated ? "Activado" : "Desactivado"}</span></div>
      </div>
