...
const panelGridClass = 'grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3';

function CategoryFilterPanel({ categoryId, categories, onChange }: { categoryId: string | null; categories: Category[]; onChange: (id: string | null) => void }) {
  return (
    <div className={panelGridClass}>
      <label className={radioLabelClass(!categoryId)}>
        <input type="radio" name="cat" checked={!categoryId} onChange={() => onChange(null)} className="sr-only" />
        <span className="text-sm font-medium">Todas</span>
      </label>
      {categories.map((c) => (
        <label key={c.id} className={radioLabelClass(categoryId === c.id)}>
          <input type="radio" name="cat" checked={categoryId === c.id} onChange={() => onChange(c.id)} className="sr-only" />
...
function BrandFilterPanel({ brandId, brands, onChange }: { brandId: string | null; brands: Brand[]; onChange: (id: string | null) => void }) {
  return (
    <div className={panelGridClass}>
      <label className={radioLabelClass(!brandId)}>
        <input type="radio" name="brand" checked={!brandId} onChange={() => onChange(null)} className="sr-only" />
        <span className="text-sm font-medium">Todas</span>
      </label>
      {brands.map((b) => (
        <label key={b.id} className={radioLabelClass(brandId === b.id)}>
          <input type="radio" name="brand" checked={brandId === b.id} onChange={() => onChange(b.id)} className="sr-only" />
  ...
  return (
    <div className={panelGridClass}>
      <label className={radioLabelClass(active === null)}>
        <input type="radio" name="act" checked={active === null} onChange={() => onChange(null)} className="sr-only" />
        <span className="text-sm font-medium">Todos</span>
      </label>
      <label className={radioLabelClass(active === true)}>
        <input type="radio" name="act" checked={active === true} onChange={() => onChange(true)} className="sr-only" />
        <span className="text-sm font-medium">Activos</span>
      </label>
      <label className={radioLabelClass(active === false)}>
        <input type="radio" name="act" checked={active === false} onChange={() => onChange(false)} className="sr-only" />
...
function FeaturedFilterPanel({ featured, onChange }: { featured: boolean | null; onChange: (v: boolean | null) => void }) {
  return (
    <div className={panelGridClass}>
      <label className={radioLabelClass(featured === null)}>
        <input type="radio" name="feat" checked={featured === null} onChange={() => onChange(null)} className="sr-only" />
        <span className="text-sm font-medium">Todos</span>
      </label>
      <label className={radioLabelClass(featured === true)}>
        <input type="radio" name="feat" checked={featured === true} onChange={() => onChange(true)} className="sr-only" />
        <span className="text-sm font-medium">Destacados</span>
      </label>
      <label className={radioLabelClass(featured === false)}>
        <input type="radio" name="feat" checked={featured === false} onChange={() => onChange(false)} className="sr-only" />
      ...
      <div className="relative z-10 bg-white rounded-none sm:rounded-lg shadow-xl w-full h-full sm:h-auto sm:max-h-[85vh] sm:max-w-2xl flex flex-col overflow-hidden">
        ...
        <div className="flex flex-col sm:flex-row flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 p-4 sm:p-6 overflow-y-auto">
            ...
            {activeSection === 'category' && <CategoryFilterPanel categoryId={categoryId} categories={categories} onChange={setCategoryId} />}
