...
function CategoryFilterPanel({ category, onChange }: { category: string | null; onChange: (c: string | null) => void }) {
  return (
    <div className="grid grid-cols-2 gap-3">
      <label className={radioLabelClass(!category)}>
        <input type="radio" name="cat" checked={!category} onChange={() => onChange(null)} className="sr-only" />
        <span className="text-sm font-medium">Todas</span>
      </label>
      {CATEGORIES.map((c) => (
        <label key={c} className={radioLabelClass(category === c)}>
          <input type="radio" name="cat" checked={category === c} onChange={() => onChange(c)} className="sr-only" />
      ...
      <div className="relative z-10 bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[80vh] flex flex-col overflow-hidden">
        ...
        <div className="flex flex-1 min-h-0 overflow-hidden">
          ...
          <div className="flex-1 p-6 overflow-y-auto">
            ...
            {activeSection === 'category' && <CategoryFilterPanel category={category} onChange={setCategory} />}
