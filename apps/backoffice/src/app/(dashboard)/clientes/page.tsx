              ...
              <tr key={c.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                <td className="px-4 py-3">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-full bg-[#0f172a] flex items-center justify-center shrink-0"><User className="w-4 h-4 text-[#C8FF00]" /></div>
                    <p className="text-gray-900 font-medium text-sm whitespace-nowrap">{c.name}</p>
                  </div>
                </td>
                <td className="px-4 py-3">
                  <div className="flex items-center gap-1.5 text-sm text-gray-500"><Mail className="w-3.5 h-3.5 shrink-0" /><span className="truncate">{c.email}</span></div>
                </td>
                <td className="px-4 py-3 text-sm text-gray-500 whitespace-nowrap">{c.phone || '-'}</td>
                <td className="px-4 py-3 text-center"><span className="text-sm font-semibold text-gray-900">{c._count?.orders || 0}</span></td>
                <td className="px-4 py-3 text-center">
                  <div className="flex items-center justify-center gap-1.5 text-xs text-gray-500 whitespace-nowrap">
                    <Calendar className="w-3 h-3 shrink-0" />{new Date(c.createdAt).toLocaleDateString('es-AR')}
                  </div>
                </td>
              </tr>
            ))}
...
function CustomersCards({ customers }: { customers: Customer[] }) {
  return (
        ...
        <div key={c.id} className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-3 flex-1 min-w-0">
              <div className="w-10 h-10 rounded-full bg-[#0f172a] flex items-center justify-center shrink-0"><User className="w-4 h-4 text-[#C8FF00]" /></div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-gray-900 truncate">{c.name}</p>
                <p className="text-xs text-gray-400 flex items-center gap-1 truncate"><Mail className="w-3 h-3 shrink-0" />{c.email}</p>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between gap-2 pt-2 border-t border-gray-100">
            ...
            <span className="text-xs text-gray-400 whitespace-nowrap shrink-0 flex items-center gap-1"><Calendar className="w-3 h-3" />{new Date(c.createdAt).toLocaleDateString('es-AR')}</span>
          </div>
        </div>
      ))}
    </div>
    ...
    <div className="space-y-6">
      ...
      {paginated.length === 0 ? (
        ...
        <>
          ...
          <CustomersCards customers={paginated} />
