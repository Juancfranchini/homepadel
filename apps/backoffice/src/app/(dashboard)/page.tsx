...
import { TrendingUp, ShoppingBag, BarChart2, Package, Star, Megaphone, Truck, TrendingDown, Wallet } from 'lucide-react';
...
function ExpensesCard({ expensesStats }: { expensesStats: any }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
      ...
      {expensesStats.byCategory.length > 0 ? (
        <div className="space-y-3">
          {expensesStats.byCategory.map((cat: any) => (
...
function OrdersByStatusCard({ ordersByStatus }: { ordersByStatus: Record<string, number> }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 shadow-sm">
        ...
        <div className="space-y-3">
          {products.map((p) => (
            <div key={p.rank} className="flex items-center gap-3">
              <span className={'w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold shrink-0 ' + (p.rank === 1 ? 'bg-[#C8FF00] text-[#0f172a]' : 'bg-gray-100 text-gray-500')}>{p.rank}</span>
...
function SummaryStatsRow({ productsStats, reviewsStats, marketingStats }: { productsStats: any; reviewsStats: any; marketingStats: any }) {
  return (
...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <KpiGrid kpis={kpis} />
      <ExpensesCard expensesStats={expensesStats} />
      ...
      <SummaryStatsRow productsStats={productsStats} reviewsStats={reviewsStats} marketingStats={marketingStats} />
    </div>
  );
}