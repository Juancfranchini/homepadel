...
function statusBadgeClass(active: boolean) {
  return 'text-xs font-medium px-2 py-1 rounded-full ' + (active ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700');
}

function SubscribersTable({ subscribers, onUnsubscribe }: { subscribers: Subscriber[]; onUnsubscribe: (email: string) => void }) {
  return (
            ...
            {subscribers.map((s) => (
              <tr key={s.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                ...
                <td className="px-4 py-3 text-center">
                  <span className={statusBadgeClass(s.active)}>{s.active ? 'Activo' : 'Inactivo'}</span>
                </td>
                <td className="px-4 py-3 text-center text-xs text-gray-500">{new Date(s.createdAt).toLocaleDateString('es-AR')}</td>
                <td className="px-4 py-3 text-center">
                  {s.active && (
                    <button onClick={() => onUnsubscribe(s.email)} className="p-1.5 rounded-lg text-red-400 hover:bg-red-50 transition-colors" title="Desuscribir">
...
function SubscribersCards({ subscribers, onUnsubscribe }: { subscribers: Subscriber[]; onUnsubscribe: (email: string) => void }) {
  return (
  ...
  return (
    <div className="space-y-6">
      ...
      {subscribers.length === 0 ? (
        ...
        <>
          <SubscribersTable subscribers={paginated} onUnsubscribe={handleUnsubscribe} />
          <SubscribersCards subscribers={paginated} onUnsubscribe={handleUnsubscribe} />
        </>
      )}
