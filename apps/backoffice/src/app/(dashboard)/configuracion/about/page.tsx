...
function BenefitRow({ index, register, currentIcon, onRemove }: { index: number; register: any; currentIcon: string; onRemove: () => void }) {
    ...
    <div className="flex gap-3 items-start p-3 bg-gray-50 rounded-lg">
      <div className="w-24">
        ...
        <div className="relative">
          <select {...register(('benefits.' + index + '.icon') as any)} className={inputClass + ' text-xs pl-8'}>
      ...
      <div className="flex-1">
        ...
        <input {...register(('benefits.' + index + '.title') as any)} className={inputClass} placeholder="Ej: Envíos a todo el país" />
      </div>
      <div className="flex-1">
        <label className="block text-xs font-medium text-gray-500 mb-1">Descripción (opciónal)</label>
        <input {...register(('benefits.' + index + '.description') as any)} className={inputClass} placeholder="Ej: Llegamos a cada rincon" />
      </div>
      <button type="button" onClick={onRemove} className="p-2 mt-5 text-red-400 hover:bg-red-50 rounded-lg transition-colors shrink-0">
...
function BenefitsSection({ fields, register, benefitsWatch, onAppend, onRemove }: {
  fields: { id: string }[]; register: any; benefitsWatch: any; onAppend: () => void; onRemove: (i: number) => void;
}) {
  return (
    <div className="border-t border-gray-100 pt-4">
      ...
      <div className="space-y-3">
        {fields.map((field, i) => (
          <BenefitRow key={field.id} index={i} register={register} currentIcon={benefitsWatch?.[i]?.icon || 'Star'} onRemove={() => onRemove(i)} />
        ))}
      </div>
    </div>
  );
...
function useAboutConfig() {
    ...
    try {
      ...
      if (data && typeof data === 'object') {
        reset({
          ...
          image: data.image ?? '',
          benefits: data.benefits?.length > 0 ? data.benefits : DEFAULT_ABOUT_BENEFITS,
          active: data.active !== false,
        });
      }
    } catch { toast('No se pudo cargar la sección "Sobre nosotros"', 'error'); } finally { setLoading(false); }
  }, [reset, toast]);
  ...
  return { form, loading, saving, onSubmit };
}

export default function AboutConfigPage() {
  const { form, loading, saving, onSubmit } = useAboutConfig();
    ...
    <div className="space-y-6 w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-6">
          ...
          <BenefitsSection fields={fields} register={register} benefitsWatch={benefitsWatch} onAppend={() => append({ icon: 'Star', title: '', description: '' })} onRemove={remove} />
        </div>
