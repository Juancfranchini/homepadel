...
function BenefitsSection({ register, watch, benefitsArray }: { register: any; watch: any; benefitsArray: any }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-3">
      ...
      {benefitsArray.fields.map((field: any, i: number) => {
...
function StringListSection({ title, register, fieldName, fieldsArray, iconBg, iconBorder, icon, placeholder }: {
  title: string; register: any; fieldName: string; fieldsArray: any; iconBg: string; iconBorder: string; icon: React.ReactNode; placeholder: string;
}) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-3">
      <div className="flex items-center justify-between border-b border-gray-100 pb-3">
        <h3 className="text-sm font-semibold text-gray-800">{title}</h3>
        <button type="button" onClick={() => fieldsArray.append('')}
          className="flex items-center gap-1 text-xs font-semibold px-2 py-1 bg-[#C8FF00] text-[#0f172a] rounded-lg hover:bg-[#b8ef00]"><Plus size={12} />Agregar</button>
      </div>
      {fieldsArray.fields.map((field: any, i: number) => (
        <div key={field.id} className="flex gap-2">
          <span className={'w-5 h-5 rounded-full ' + iconBg + ' border ' + iconBorder + ' flex items-center justify-center flex-none mt-0.5'}>{icon}</span>
          <input {...register(fieldName + '.' + i)} className={inputClass} placeholder={placeholder} />
          <button type="button" onClick={() => fieldsArray.remove(i)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg"><Trash2 size={14} /></button>
        </div>
      ))}
    </div>
  );
...
function StepsSection({ register, stepsArray }: { register: any; stepsArray: any }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-3">
      <div className="flex items-center justify-between border-b border-gray-100 pb-3">
      ...
      {stepsArray.fields.map((field: any, i: number) => (
        <div key={field.id} className="flex gap-2">
...
function HelpSection({ register }: { register: any }) {
  return (
...
export default function PoliticaDevoluciónPage() {
  ...
  const { register, handleSubmit, reset, control, watch } = useForm<any>({ resolver: zodResolver(z.object({}).passthrough()) });
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <BenefitsSection register={register} watch={watch} benefitsArray={benefitsArray} />
        <StringListSection title="Condiciones para devolución" register={register} fieldName="conditionsOk" fieldsArray={conditionsOkArray}
          iconBg="bg-[#B7D31A]/10" iconBorder="border-[#B7D31A]/30" icon={<Check size={10} className="text-[#B7D31A]" />} placeholder="Condicion..." />
        <StepsSection register={register} stepsArray={stepsArray} />
        <StringListSection title="Cuando NO aplica" register={register} fieldName="conditionsNo" fieldsArray={conditionsNoArray}
          iconBg="bg-red-500/10" iconBorder="border-red-500/20" icon={<X size={10} className="text-red-400" />} placeholder="No aplica si..." />
        <HelpSection register={register} />
