...
function CtaColumn({ register, errors }: { register: any; errors: any }) {
  return (
...
function NewsletterColumn({ register, errors }: { register: any; errors: any }) {
  return (
    <div className="bg-white rounded-xl border border-gray-200 p-6 space-y-4">
      <h2 className="text-sm font-semibold text-gray-800 border-b border-gray-100 pb-3 flex items-center gap-2">
...
const DEFAULT_CTA_NEWSLETTER: FormData = {
...
export default function CtaNewsletterConfigPage() {
  ...
  const { register, handleSubmit, reset, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: DEFAULT_CTA_NEWSLETTER,
  });
  ...
  const load = useCallback(async () => {
    ...
    } catch { toast('No se pudo cargar el mensaje final', 'error'); } finally { setLoading(false); }
  }, [reset]);
    ...
    <div className="space-y-6 w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <CtaColumn register={register} errors={errors} />
          <NewsletterColumn register={register} errors={errors} />
        </div>
