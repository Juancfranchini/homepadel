...
import { z } from 'zod';
import { Save, ArrowLeft, Cloud } from 'lucide-react';
...
function CloudinaryFields({ register, errors }: { register: UseFormRegister<FormData>; errors: FieldErrors<FormData> }) {
  return (
...
export default function CloudinaryConfigPage() {
  ...
  const load = useCallback(async () => {
    ...
    } catch { toast('No se pudo cargar la configuración de Cloudinary', 'error'); } finally { setLoading(false); }
  }, [reset]);
    ...
    <div className="space-y-6 w-full">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        <div className="bg-white rounded-xl border border-gray-200 px-4 sm:px-6 py-4 sm:py-6 space-y-4">
          ...
          <CloudinaryFields register={register} errors={errors} />
        </div>
