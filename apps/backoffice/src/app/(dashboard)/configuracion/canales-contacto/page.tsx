...
function ChannelCard({ channel, onEdit, onDelete }: { channel: any; onEdit: () => void; onDelete: () => void }) {
  const IconComp = ICON_MAP[channel.icon] || MessageCircle;
  return (
    <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-gray-900 font-medium text-sm">{channel.title || 'Sin titulo'}</p>
            <p className="text-xs text-gray-400">{channel.desc || ''}</p>
          </div>
        </div>
      </div>
      {channel.detail && <p className="text-sm text-gray-600">{channel.detail}</p>}
      {channel.href && <a href={channel.href} target="_blank" rel="noopener noreferrer" className="text-xs text-[#C8FF00] hover:underline break-all">{channel.href}</a>}
      <div className="border-t pt-2 flex justify-end gap-2">
        <button onClick={onEdit} className="p-1.5 rounded-lg text-[#C8FF00] hover:bg-[#C8FF00]/10"><Edit2 className="w-4 h-4" /></button>
        <button onClick={onDelete} className="p-1.5 rounded-lg text-red-400 hover:bg-red-400/10"><Trash2 className="w-4 h-4" /></button>
      </div>
    </div>
  );
}

function ChannelFormModal({ isOpen, editItem, form, onFormChange, onClose, onSubmit, saving }: {
  isOpen: boolean; editItem: any; form: any; onFormChange: (f: any) => void; onClose: () => void; onSubmit: () => void; saving: boolean;
}) {
  if (!isOpen) return null;
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={editItem ? 'Editar canal' : 'Nuevo canal'} size="sm">
      <div className="space-y-4">
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Icono</label>
          <select value={form.icon} onChange={(e) => onFormChange({ ...form, icon: e.target.value })} className={inputClass}>
            {ICON_OPTIONS.map((opt) => <option key={opt.value} value={opt.value}>{opt.label}</option>)}
          </select>
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Titulo</label>
          <input value={form.title} onChange={(e) => onFormChange({ ...form, title: e.target.value })} className={inputClass} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Descripcion</label>
          <input value={form.desc} onChange={(e) => onFormChange({ ...form, desc: e.target.value })} className={inputClass} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Detalle</label>
          <input value={form.detail} onChange={(e) => onFormChange({ ...form, detail: e.target.value })} className={inputClass} />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-1">Link</label>
          <input value={form.href} onChange={(e) => onFormChange({ ...form, href: e.target.value })} className={inputClass} />
        </div>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {channels.map((c) => (
          <ChannelCard key={c.id} channel={c} onEdit={() => openEdit(c)} onDelete={() => setDeleteTarget(c)} />
        ))}
      </div>

      <ChannelFormModal isOpen={modalOpen} editItem={editItem} form={form} onFormChange={setForm} onClose={() => setModalOpen(false)} onSubmit={onSubmit} saving={saving} />
