...
function StoreInfoSection({ generalForm }: { generalForm: any }) {
  const { errors } = generalForm.formState;
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
        <div>
          ...
          {errors.storeName && <p className="text-xs text-red-600 mt-1">{errors.storeName.message}</p>}
        </div>
        <div>
...
function LogoCard({ logo, generalForm, uploading, uploaded, onUploadClick }: {
  logo: typeof LOGO_MAP[number]; generalForm: any; uploading: boolean; uploaded: boolean; onUploadClick: () => void;
}) {
  ...
  return (
    <div className="border border-gray-100 rounded-xl p-4">
      ...
      <div className="flex gap-2">
        ...
        >
          {uploading ? '...' : uploaded ? <><Check className="w-4 h-4" />Subido</> : <><Upload className="w-4 h-4" />Subir</>}
        </button>
      </div>
...
export default function GeneralTab({ generalForm, onSave, saving }: GeneralTabProps) {
  ...
  return (
    <div className="space-y-6">
      <StoreInfoSection generalForm={generalForm} />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
          {LOGO_MAP.map((logo) => (
            <LogoCard
              ...
              uploading={uploading === logo.key}
              uploaded={!!uploadedKeys[logo.key]}
              onUploadClick={() => triggerUpload(logo.key)}
            />
          ))}
        </div>
      </div>
