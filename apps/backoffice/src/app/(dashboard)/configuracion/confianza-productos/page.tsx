...
function PreviewGrid({ items }: { items: { icon: string; title: string; subtitle?: string }[] | undefined }) {
  return (
...
function TrustItemRow({ index, register, currentIcon, titleError, onRemove }: {
  index: number; register: any; currentIcon: string; titleError?: string; onRemove: () => void;
}) {
    ...
    <div className="flex gap-3 items-start p-3 bg-gray-50 rounded-lg">
      <div className="w-32">
        ...
        <div className="relative">
          <select {...register(('items.' + index + '.icon') as any)} className={inputClass + ' text-xs pl-8'}>
      ...
      <div className="flex-1">
        ...
        <input {...register(('items.' + index + '.title') as any)} className={inputClass} placeholder="ENVÍOS A TODO EL PAÍS" />
        {titleError && <p className="text-xs text-red-600 mt-0.5">{titleError}</p>}
      </div>
      <div className="flex-1">
        <label className="block text-xs font-medium text-gray-500 mb-1">Subtítulo</label>
        <input {...register(('items.' + index + '.subtitle') as any)} className={inputClass} placeholder="Recibi tu pedido en..." />
      </div>
...
export default function ConfianzaProductosPage() {
  ...
  const { register, handleSubmit, reset, control, watch, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { items: DEFAULT_TRUST_ITEMS, active: true },
  });
  ...
  const load = useCallback(async () => {
    ...
    } catch { toast('No se pudo cargar "Confianza Productos"', 'error'); } finally { setLoading(false); }
  }, [reset]);
          ...
          <div className="space-y-3">
            {fields.map((field, i) => (
              <TrustItemRow
                ...
                index={i}
                register={register}
                currentIcon={itemsWatch?.[i]?.icon || 'Shield'}
                titleError={errors.items?.[i]?.title?.message}
                onRemove={() => remove(i)}
              />
            ))}
          </div>
