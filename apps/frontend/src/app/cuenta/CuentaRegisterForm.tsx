...
export default function CuentaRegisterForm({ form, onSubmit, showPassword, onToggleShowPassword }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
      <div>
        <label htmlFor="register-name" className="block text-xs font-semibold text-[#8A8A85] uppercase tracking-wide mb-1">Nombre completo</label>
        <div className="relative"><User size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8A8A85]" /><input id="register-name" {...register('name')} type="text" autoComplete="name" placeholder="Escribí tu nombre completo" className={errors.name ? errorInputClass : inputClass} /></div>
        {errors.name && <p className="text-red-500 text-xs mt-1">{String(errors.name.message)}</p>}
      </div>
      <div>
        <label htmlFor="register-email" className="block text-xs font-semibold text-[#8A8A85] uppercase tracking-wide mb-1">Email</label>
        <div className="relative"><Mail size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8A8A85]" /><input id="register-email" {...register('email')} type="email" autoComplete="email" placeholder="Escribí tu email" className={errors.email ? errorInputClass : inputClass} /></div>
        {errors.email && <p className="text-red-500 text-xs mt-1">{String(errors.email.message)}</p>}
      </div>
      <div>
        <label htmlFor="register-password" className="block text-xs font-semibold text-[#8A8A85] uppercase tracking-wide mb-1">Contraseña</label>
        <div className="relative"><Lock size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#8A8A85]" /><input id="register-password" {...register('password')} type={showPassword ? 'text' : 'password'} autoComplete="new-password" placeholder="Creá una contraseña de 6 caracteres o más" className={errors.password ? errorInputClass : inputClass} />
