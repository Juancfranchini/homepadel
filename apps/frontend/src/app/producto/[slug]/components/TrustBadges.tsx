...
export default function TrustBadges() {
  return (
    <div className="grid grid-cols-3 gap-1.5 sm:gap-2 py-2 sm:py-3 border-y border-[#0D0F0F]">
      {[
        ...
        <div key={i} className="flex items-center gap-1 sm:gap-2">
          ...
          <p className="text-[#F7F6F7] font-semibold text-[9px] sm:text-xs leading-tight whitespace-nowrap">{t.title}</p>
        </div>
      ))}
    </div>
  );
