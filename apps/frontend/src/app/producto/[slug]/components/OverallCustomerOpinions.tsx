...
import AverageStarRow from './AverageStarRow';
import ReviewsInfoModal from './ReviewsInfoModal';
...
export default function OverallCustomerOpinions({ productId }: Props) {
  ...
  return (
    <div className="bg-[#0C0C0C] rounded-xl border border-[#0D0F0F] p-5 h-full flex flex-col relative">
      ...
      {/* Modal info */}
      {showInfo && (
        <ReviewsInfoModal title={infoTitle} text={infoText} onClose={() => setShowInfo(false)} />
      )}
    </div>
  );
}
