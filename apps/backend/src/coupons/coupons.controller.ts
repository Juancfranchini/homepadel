...
export class CouponsController {
  ...
  @Post('validate')
  async validate(@Body('code') code: string, @Body('subtotal') subtotal?: number) {
    const coupon = await this.couponsService.validate(code, subtotal);
    const discountAmount = subtotal !== undefined ? this.couponsService.calculateDiscount(coupon, subtotal) : undefined;
