...
export class DashboardService {
  ...
  async getStats() {
    ...
    return {
      kpis: sales.kpis,
      expensesStats: sales.expensesStats,
      ...
      productsSoldCount: productStats.productsSoldCount,
      ...(await this.getCounts(thirtyDaysAgo)),
    };
  }
  ...
  private async getSalesData(thirtyDaysAgo: Date) {
    ...
    return {
      allOrders,
      kpis: { ventas30, totalPedidos, ticketPromedio, ganancia, gastos30 },
      expensesStats: {
        total30: gastos30,
  ...
  private getProductStats(allOrders: Awaited<ReturnType<DashboardService['getSalesData']>>['allOrders']) {
  ...
  private async getCounts(thirtyDaysAgo: Date) {
    return {
