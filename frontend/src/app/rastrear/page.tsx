...
interface TrackedOrder {
  ...
  buyerPhone?: string;
  buyerName?: string;
  items: { quantity: number; product: { name: string; images: string[] }; variant?: { sku: string; size: string; color?: string | null } | null }[];
}
              ...
              <div className="flex items-center justify-between pt-4 border-t border-[#0D0F0F]">
                <div><p className="text-xs text-[#8A8A85] uppercase tracking-wide">Productos</p><p className="text-sm font-semibold text-[#F7F6F7]">{order.items.map((item, i) => (
                  <span key={i} className="text-sm font-semibold text-[#F7F6F7]">
                    {item.product.name}
                    {item.variant && <span className="text-[#8A8A85] font-normal"> ({item.variant.size}{item.variant.color ? ' / ' + item.variant.color : ''})</span>}
